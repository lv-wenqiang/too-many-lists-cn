...
让我们的一个元素不在堆中分配，或许也比所有元素都在堆中分配更糟。这是因为它给了我们一个*不一致的*节点内存布局。在压入和弹出节点时这并无问题，但在分割和合并列表时确实会有影响。
...
不过它确实有*一个*有趣的特性：它完全避免了在堆里分配Empty，让堆内存分配的数量减少了1。不幸的是，这么做反而浪费了*更多空间*！因为之前的布局利用了*空指针优化*（null pointer optimization）。

我们之前了解到每个enum需要存储一个*标签*，来指明它代表哪一个enum的*变体*（variant）。然而，如果我们有如下特殊类型的enum：
...
你还能想到其他可以进行这种优化的枚举和类型吗？其实有很多！这就是 Rust 完全不规定枚举布局的原因。Rust 还会为我们执行一些更复杂的枚举布局优化，不过空指针优化绝对是最重要的一个！这意味着 `&`、`&mut`、`Box`、`Rc`、`Arc`、`Vec` 以及 Rust 中其他几个重要类型放进 `Option` 时都没有额外开销！（其中大多数我们会在适当的时候讲到。）
